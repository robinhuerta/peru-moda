      ...
      <div className="group relative overflow-hidden">
        ...
        <div className="flex w-max animate-marquee gap-16 group-hover:[animation-play-state:paused]">
          {track.map((product, index) => (
            ...
            >
              <span className="relative h-16 w-16 overflow-hidden rounded-full border border-white/15">
                <Image src={product.image} alt={product.name} fill className="object-cover" sizes="64px" />
              </span>
              <span className="text-center text-xs text-slate-400">{product.name}</span>
            </Link>
          ))}
        </div>
      </div>
